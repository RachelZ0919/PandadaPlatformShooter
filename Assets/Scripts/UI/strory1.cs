...
{
public class strory1 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        computer=GameObject.Find("computer").GetComponent<Image>();
    ...
    void ChangeColor() 
    {
        ...
        else
        {
            //透明度降低
            UI_Alpha=0;
            canvasGroup.alpha = Mathf.Lerp(canvasGroup.alpha, UI_Alpha, -50.0f*alphaSpeed * Time.deltaTime);
            if (Mathf.Abs(UI_Alpha - canvasGroup.alpha) <= 0.01f)
            {
                canvasGroup.alpha = UI_Alpha;
                //显示屏上移，相机缩放
                if(computer.transform.position.y<=0.01f)
                {
                    Vector3 offset = new Vector3(0,0.1f,0);
                    computer.transform.position=computer.transform.position+offset;
                }
               
                if(computer.transform.position.y>0.01f&& computer.transform.localScale.sqrMagnitude<=75)
                {
                   computer.transform.localScale +=new Vector3 (0.1f, 0.1f, 0.1f);
                }
                if(computer.transform.localScale.sqrMagnitude>75)
                {
                    //切换场景
                    switchScene("GameLogin");
                }
